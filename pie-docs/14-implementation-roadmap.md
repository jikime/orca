...
R7 채팅의 제품 범위와 완료 판정은 [채팅 핵심 제품 로드맵](./36-chat-core-roadmap.md)이 소유한다.
Slack 전체 복제가 아니라 타임라인 신뢰성, 메시지 소유권, 스레드·멘션 완결, Pie 업무 연결
순서로 구현한다.
...
pie:// 딥링크[R1 `pieAuthCallbackBroker`]를 한 인터페이스 뒤에 두고 fallback), `token-exchange`(authorization\_
...
2026-07-21 Desktop 프로젝트 작업면을 R4 Core API에 연결했다. `프로젝트`는 개요·작업·납품·품질·관리
탭 안에서 하나의 선택 프로젝트 문맥을 유지하고, 개요에서 WorkItem·변경 요청·결과물·결함·위험·최근
상태보고서와 결정을 요약한다. 프로젝트와 하위 관리 자원의 생성·편집은 Dialog로 통일하고 PATCH에는
resource version 기반 ETag를 사용한다. 승인·검수·완료 전이는 기존 action과 분리하며, 결정은 불변 기록으로
유지한다. 프로젝트 canonical 대화와 연결 회의·예약 진입점도 개요에 보존했다. 상세 필드·API·자동화
증거와 남은 범위는 [프로젝트 작업면 기능 가이드](./41-project-workspace-feature-guide.md)가 소유한다.
...
2026-07-20 meeting vertical은 lifecycle·초대 gate·LiveKit room token·다자간 음성/영상·화면 공유·
카메라 선택·갤러리/발언자 무대·녹화 동의·MP4/MP3 egress·실시간 자막·사후 전사·AI 회의록·사람
검토/확정과 revision의 얇은 수직을 구현했다. 이는 R7 회의 완료가 아니다. 구조화 transcript segment와
근거, 결정·액션 엔터티와 WorkItem 연결, capture별 consent/retention/delete, host control, calendar/guest,
recap/search가 남는다. 현재 기준과 M0~M7 순서는
[회의·회의록 제품 로드맵](./35-meeting-product-roadmap.md)이 소유한다.

2026-07-21 meeting vertical은 시간·화자별 transcript segment와 correction revision, 근거가 연결된
결정·액션 엔터티와 승인 후 WorkItem 전환, camera/mic/speaker 사전 점검, host mute/remove, 일정 입력과
10분 전 Desktop 알림, 종료 후 Recap까지 확장했다. 이어 capture별 consent와 정책 version, 캡처
pause/resume/stop, 새 참여자·동의 철회 차단, retention·lease 기반 자동 삭제, 객체와 파생 데이터 연쇄
삭제, legal hold, 감사되는 JSON export를 구현했다. 대기실·공동 호스트·발표자별 LiveKit publish source
축소와 reconnecting/degraded/recovered 표시도 M4에서 구현했다. M5에서 IANA 시간대·반복,
내부 사용자 검색, guest link·session, Google·Microsoft outbound calendar adapter와
Project·Ticket·RemoteSession 역링크를 구현했다. 실장비 media E2E, guest Desktop 소비 흐름,
calendar OAuth lifecycle, 부분 redaction·관리자 step-up·backup 복원 후 재삭제는 운영 완결
milestone로 남는다. M6 회의록 생산성과 M7 누적 지식은 다른 메뉴 개발을 위해 보류했다.

2026-07-21 채팅 안정성·운영 보강으로 사용자·채널·스레드별 초안 복원, 동일 요청 ID 전송 재시도,
서버 read cursor 기반 unread 경계와 스크롤 복원, 150건 이상 타임라인 가상화, 채널별 1~3650일
보존·관리자 JSON 내보내기·감사 조회·타인 메시지 삭제 사유를 Desktop까지 연결했다. 보존 적용은
멱등 mutation이며 실제 PostgreSQL vertical에서 본문·revision redaction, 권한 거부와 감사 사유를 함께
검증한다. 상세 gate는 [채팅 릴리스 검증 기록](./37-chat-release-evidence.md)이 소유한다.
