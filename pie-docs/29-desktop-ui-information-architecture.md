...
새 기능을 별도 관리 앱처럼 중첩하지 않는다. 현재 3열 셸을 유지하고 기존 왼쪽 사이드바 상단에서
`Orca | Pie` 모드를 전환한다. Pie를 선택하면 기존 Workspace 목록 대신 Pie 모듈 메뉴를 같은
사이드바에 표시하고, 중앙과 오른쪽 영역의 내용만 선택한 모듈에 맞게 교체한다.

## 셸 구조

```text
┌───────────────────────────┬──────────────────────────────┬──────────────────┐
│ 왼쪽 사이드바             │ 주 작업 영역                 │ 우측 인스펙터    │
│ [ Orca ] [ Pie ]          │                              │                  │
│ 220~500px                 │ 가변                         │ 340~420px        │
├───────────────────────────┴──────────────────────────────┴──────────────────┤
...
| 영역 | 책임 |
|---|---|
| 왼쪽 사이드바 | Orca/Pie 모드 전환과 선택한 모드의 탐색, 목록, 저장된 보기와 필터 |
...
사이드바의 위치와 폭은 모드 전환 중 바뀌지 않는다. 각 모드는 마지막으로 보던 화면을 기억한다.
사이드바와 인스펙터를 열고 닫아도 중앙 작업면의 선택, scroll과 query 상태를 잃지 않는다.

## 사이드바 모드 탭

`일반`처럼 Pie를 예외 기능으로 보이게 하는 이름을 쓰지 않는다. 브랜드와 책임 경계를 그대로
드러내는 `Orca | Pie` 두 탭을 같은 너비로 배치한다.
...
Pie 탭 안에는 다음 모듈을 배치한다. 역할과 permission에 없는 모듈은 빈 자리 없이 제거한다.

| Pie 모듈 | 기본 사용자 | 주요 작업면 |
|---|---|---|
...
Pie 화면은 주 작업 영역만 렌더링한다. Pie 안에 모듈 사이드바를 다시 만들지 않는다. 채팅·회의 알림과
WorkItem 딥링크는 Pie 탭과 대상 메뉴를 함께 활성화하며, Orca로 돌아오면 직전에 보던 Orca 화면을
복원한다.
...
첫 제품 단계에서는 세부 도메인을 같은 수준의 메뉴로 노출하지 않고 `My Work`와 `Projects`만 기본
노출한다. Team은 기본 Team 하나를 자동 적용하고, Inbox·Intake는 미분류 외부 요청을 실제로 운영할
때 활성화한다. 세부 단계와 WorkItem·Workspace 연결 순서는
[프로젝트·작업·Workspace 통합](./39-project-workspace-integration.md)을 따른다.
...
[필요할 때 확장]
  Inbox / Intake
  Views / Teams / Cycles / Initiatives
```
...
- 넓은 창에서는 왼쪽 사이드바, 중앙 작업면과 인스펙터를 동시에 표시한다.
- 공간이 부족하면 인스펙터를 overlay로 전환하고, 다음으로 왼쪽 사이드바를 접는다. 사이드바
  재열기 제어와 현재 resource 제목은 마지막까지 유지한다.
...
1. 현재 App 셸에 `Orca | Pie` 모드 탭과 공유 Pie route state를 추가한다.
2. Workspace subtree를 유지한 채 Portal surface로 왕복하는 상태 보존을 검증한다.
3. Pie 탭 내비게이션과 `My Work` List를 구현한다.
...
- 역할별로 Pie 탭 내비게이션, 빠른 열기와 명령 팔레트의 항목이 일관되게 제한된다.
