...
import { app, BrowserWindow, dialog, ipcMain, nativeTheme, safeStorage, type Tray } from 'electron'
...
function startTerminalRuntimeStartupServices(): Promise<void> {
  ...
  const startupServices = startFirstWindowStartupServices({
    // Why: both desktop and headless serve need the persistent provider before creating terminals; safe mode skips a crash-looping daemon.
    ...
    // Why: restored terminals need live hook env; safe mode skips the hook server during crash recovery.
